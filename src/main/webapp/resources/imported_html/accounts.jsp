    ...
    <c:if test="${userType == employeeId}">
        ...
        <c:forEach items="${accountsToOpen}" var="account">
            ...
            <div class="account" id="${account.id}" style="display: inline-block">
                ...
                <a class="button_close_account" href="/account?action=refuse_to_open&id=${account.id}">×</a>
                <a class="button_open_account" href="/account?action=apply_to_open&id=${account.id}">✓</a>
            </div>
        </c:forEach>
        <div class="request">
        ...
        <c:forEach items="${accountsToClose}" var="account">
            ...
            <div class="account" id="${account.id}" style="display: inline-block">
                ...
                <a class="button_close_account" href="/account?action=refuse_to_close&id=${account.id}">×</a>
                <a class="button_open_account" href="/account?action=apply_to_close&id=${account.id}">✓</a>
            </div>
        </c:forEach>
    </c:if>
